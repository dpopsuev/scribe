...
func TestRecall_FindsRelevantNote(t *testing.T) {
	...
	_, err := proto.CreateArtifact(ctx, parchment.CreateInput{Labels: []string{parchment.LabelPrefixKind + "knowledge.note"},
...
func TestRecall_EvergreenRanksHigher(t *testing.T) {
	...
	fleeting, _ := proto.CreateArtifact(ctx, parchment.CreateInput{Labels: []string{parchment.LabelPrefixKind + "knowledge.note", "note.fleeting"}, Title: "template conformance fleeting",

		Sections: []parchment.Section{{Name: "body", Text: "template conformance check fires on promote"}},
	})
	evergreen, _ := proto.CreateArtifact(ctx, parchment.CreateInput{Labels: []string{parchment.LabelPrefixKind + "knowledge.note", "note.evergreen"}, Title: "template conformance evergreen",
...
func TestRecall_RecentRanksHigher(t *testing.T) {
	...
	older, _ := proto.CreateArtifact(ctx, parchment.CreateInput{Labels: []string{parchment.LabelPrefixKind + "knowledge.note"}, Title: "wikilink resolution old",
	...
	newer, _ := proto.CreateArtifact(ctx, parchment.CreateInput{Labels: []string{parchment.LabelPrefixKind + "knowledge.note"}, Title: "wikilink resolution new",
...
func TestRecall_LimitTopN(t *testing.T) {
	...
	for i := range 8 {
		_, _ = proto.CreateArtifact(ctx, parchment.CreateInput{Labels: []string{parchment.LabelPrefixKind + "knowledge.note"},
