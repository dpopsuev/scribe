...
- Name: graph.spec.ts >> graph rendering pipeline >> nodes are positioned on a sphere — not collapsed to a line
- Location: graph.spec.ts:101:3

# Error details

```
Error: nodes collapsed to a single Z — sphere layout broken
