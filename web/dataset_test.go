...
func seedDataset(t *testing.T, s parchment.Store) *parchment.Protocol {
	...
	arts := []*parchment.Artifact{
		{ID: "TASK-001", Labels: []string{"kind:effort.task", "work.active", "scope:alpha"}, Title: "Build export"},
		{ID: "TASK-002", Labels: []string{"kind:effort.task", "work.draft", "scope:alpha"}, Title: "Draft task — excluded"},
		{ID: "ADR-001", Labels: []string{"kind:intent.decision", "decision.accepted", "scope:alpha"}, Title: "Use JSONL",
		...
		{ID: "SPEC-001", Labels: []string{"kind:intent.spec", "work.active", "scope:alpha"}, Title: "Dataset API",
			Sections: []parchment.Section{
...
func TestDatasetExport_QualityFilter_ExcludesViolation(t *testing.T) {
	...
	_ = s.Put(ctx, &parchment.Artifact{
		...
		Labels: []string{"kind:effort.task", "work.active", "compliance:violation", "scope:x"},
	})
	_ = s.Put(ctx, &parchment.Artifact{Labels: []string{"kind:effort.task", "work.active", "scope:x"}, ID: "GOOD-001", Title: "Good"})
