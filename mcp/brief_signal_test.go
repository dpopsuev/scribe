...
func seedBriefNoise(t *testing.T, s parchment.Store) {
	...
	for i := 0; i < 5; i++ {
		_ = s.Put(ctx, &parchment.Artifact{Labels: []string{"kind:ref", "work.draft", "scope:test"}, ID: "REF-" + string(rune('A'+i)), Title: "Reference " + string(rune('A'+i)),
			UpdatedAt: old})
		_ = s.Put(ctx, &parchment.Artifact{Labels: []string{"kind:doc", "work.draft", "scope:other-scope"}, ID: "DOC-" + string(rune('A'+i)), Title: "Doc " + string(rune('A'+i)),
			UpdatedAt: old})
	}
	// Tasks without sections — trigger should-section warnings
	for i := 0; i < 3; i++ {
		_ = s.Put(ctx, &parchment.Artifact{Labels: []string{"kind:task", "work.draft", "scope:test"}, ID: "TSK-" + string(rune('A'+i)), Title: "Task " + string(rune('A'+i)),
			UpdatedAt: old})
	}
}
