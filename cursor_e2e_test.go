...
func TestCursorSDKCanary(t *testing.T) {
	if os.Getenv("RUN_CURSOR_E2E") == "" {
		t.Skip("set RUN_CURSOR_E2E=1 to enable the Cursor SDK canary")
	}
	...
	if _, err := os.Stat(filepath.Join(repoRoot, "agent_e2e", "node_modules", "@cursor", "sdk", "package.json")); err != nil {
		t.Skip("Cursor SDK not installed; run `make cursor-e2e-setup` first")
	}

	workdir := createWorkspace(t)
	baseURL, logs := startScribeServer(t, repoRoot)

	title := fmt.Sprintf("Cursor SDK canary %d", time.Now().UnixNano())
	scope := "cursor-e2e"

	runCursorCanary(t, repoRoot, baseURL, workdir, title, scope, logs)
...
func createWorkspace(t *testing.T) string {
	t.Helper()
	dir := t.TempDir()
	if err := os.WriteFile(filepath.Join(dir, "README.md"), []byte("# Cursor SDK canary\n"), 0o644); err != nil {
...
func startScribeServer(t *testing.T, repoRoot string) (string, *bytes.Buffer) {
	...
	dbPath := filepath.Join(t.TempDir(), "cursor-e2e.db")
...
func runCursorCanary(t *testing.T, repoRoot, baseURL, workdir, title, scope string, logs *bytes.Buffer) {
	t.Helper()
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Minute)
	defer cancel()

	cmd := exec.CommandContext(ctx, "npm", "run", "--silent", "cursor-canary")
	cmd.Dir = filepath.Join(repoRoot, "agent_e2e")
	cmd.Env = append(os.Environ(),
		"CURSOR_CANARY_SCRIBE_URL="+baseURL,
		"CURSOR_CANARY_WORKDIR="+workdir,
		"CURSOR_CANARY_TITLE="+title,
		"CURSOR_CANARY_SCOPE="+scope,
		"CURSOR_CANARY_PRIORITY=medium",
	)

	out, err := cmd.CombinedOutput()
	if len(out) > 0 {
		t.Logf("cursor canary output:\n%s", string(out))
	}
	if ctx.Err() == context.DeadlineExceeded {
		t.Fatalf("cursor canary timed out after %s\noutput:\n%s\nscribe logs:\n%s", 5*time.Minute, string(out), logs.String())
	}
	if err != nil {
		t.Fatalf("cursor canary failed: %v\noutput:\n%s\nscribe logs:\n%s", err, string(out), logs.String())
	}
}
